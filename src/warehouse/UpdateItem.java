...
public class UpdateItem extends javax.swing.JFrame {
    ...
    private void updateButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_updateButtonActionPerformed
        ...
        if (check == 0){
            try {
                Connection conn = ConnectDataBase.connect();
                PreparedStatement ps = conn.prepareStatement("update product set productID = ? , parcode = ? , productName = ? , companyID = ? , storageMethod = ? , type = ? , cost = ? , expireDate = ? , quantity = ? , sectionID = ? where productID = ?  ");
                String productID = table.getValueAt(x, 0).toString();
                if (Controller.validateIntegerNumber(productID))
                {
                    ps.setString(1, productID);
                }
                String parcode = table.getValueAt(x, 1).toString();
                  if (Controller.validateIntegerNumber(parcode))
                  {
                      ps.setString(2, parcode);
                  }
                String productName = table.getValueAt(x, 2).toString();
                if (Controller.validateName(productName))
                {
                    ps.setString(3, productName);
                }
                String companyID = table.getValueAt(x, 3).toString();
                 if (Controller.validateIntegerNumber(companyID))
                  {
                      ps.setString(4, companyID);
                  }
                String storageMethod = table.getValueAt(x, 4).toString();
                ps.setString(5, storageMethod);
                String type = table.getValueAt(x, 5).toString();
                 if (Controller.validateName(type))
                {
                    ps.setString(6, type);
                }
                String cost = table.getValueAt(x, 6).toString();
                 if (Controller.validateFractionNumber(cost))
                  {
                      ps.setString(7, cost);
                  }
                String expireDate = table.getValueAt(x, 7).toString();
                ps.setString(8, expireDate);
                String quantity = table.getValueAt(x, 8).toString();
                 if (Controller.validateIntegerNumber(quantity))
                  {
                      ps.setString(9, quantity);
                  }
                String sectionID = table.getValueAt(x, 9).toString();
                 if (Controller.validateIntegerNumber(sectionID))
                  {
